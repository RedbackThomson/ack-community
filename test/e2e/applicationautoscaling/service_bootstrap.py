...
from applicationautoscaling.bootstrap_resources import TestBootstrapResources, SAGEMAKER_SOURCE_DATA_BUCKET


def create_execution_role() -> str:
    region = get_aws_region()
    role_name = random_suffix_name(f"ack-applicationautoscaling-execution-role", 63)
    iam = boto3.client("iam", region_name=region)

    iam.create_role(
        RoleName=role_name,
        AssumeRolePolicyDocument=json.dumps({"Version": "2012-10-17", "Statement": [
                                            {"Effect": "Allow", "Principal": {"Service": "applicationautoscaling.amazonaws.com"}, "Action": "sts:AssumeRole"}]}),
        Description="Application Auto Scaling execution role for ACK integration and canary tests"
    )

    iam.attach_role_policy(
        RoleName=role_name,
        PolicyArn='arn:aws:iam::aws:policy/AmazonApplication Auto ScalingFullAccess'
    )
    iam.attach_role_policy(
        RoleName=role_name,
    ...
    logging.info(f"Created Application Auto Scaling execution role {resource_arn}")
...
def create_data_bucket() -> str:
    ...
    logging.info(f"Created Application Auto Scaling data bucket {bucket_name}")
